...
def get_highest_scorer():
    try:
        ...
        with sqlite3.connect(ODDS_DB_PATH) as conn:
            ...
            for row in cursor.fetchall():
                teams.append(
                    {"owner": row["owner"], "win_prob": round(row["probability"] * 100, 1), "odds": row["odds"]}
                )
    ...
    try:
        ...
        for row in raw_results:
            ...
            if slug:
                ...
                print(
                    f"[TEAMS WARNING] Skipping roster_id {roster_id} - no slug (username={username}, display_name={display_name})"
                )
...
def get_team_players():
    ...
    try:
        ...
        print(
            f"[TEAM_PLAYERS SUCCESS] User {current_user.id} - Team '{team_owner}' (roster_id={roster_id}): {len(starters)} starters, {len(bench)} bench"
        )
        return jsonify({"starters": starters, "bench": bench})

    except Exception as e:
        print(
            f"[TEAM_PLAYERS ERROR] User {current_user.id} - Exception for team '{team_owner}': {type(e).__name__}: {str(e)}"
        )
