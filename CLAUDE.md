...
**Auth**: Google OAuth via flask-dance (`auth.py`). Admin access controlled by `ADMIN_EMAILS` env var. `replit_auth.py` is legacy — do not use.

## Commands

- **Run app**: `python app.py` (Flask on 0.0.0.0:5000)
...
Production uses Neon PostgreSQL (connection via `NEON_DATABASE_URL` in `.env`).
...
- **`.env` required** — needs `SECRET_KEY`, `DATABASE_URL`, `GOOGLE_OAUTH_CLIENT_ID`, `GOOGLE_OAUTH_CLIENT_SECRET`, `ADMIN_EMAILS` at minimum.
